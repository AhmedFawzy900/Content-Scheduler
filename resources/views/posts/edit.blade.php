            ...
            <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="scheduled_time" id="scheduled_time" 
                        value="{{ old('scheduled_time', $post->scheduled_time->format('Y-m-d\TH:i')) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <div class="mt-2 space-y-2">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="platform_ids[]" value="{{ $platform->id }}" id="platform_{{ $platform->id }}"
                                ...
                                {{ in_array($platform->id, old('platform_ids', $post->postPlatforms->pluck('platform_id')->toArray())) ? 'checked' : '' }}>
